...
SpiSlave::SpiSlave(const int sck_pin, const int miso_pin, const int mosi_pin, const int ss_pin,
    ...
    */
    ...
    _rx_buffer = calloc(_commands_size, sizeof *_rx_buffer);
    _tx_buffer = calloc(_commands_size, sizeof *_tx_buffer);
}
...
ISR (SPI_STC_vect) {
    ...
    if (full_command_or_synchronized && previously_synchronized) {
        ...
        const bool command_interpreted = SpiCommands::slave_interpret_command(_rx_buffer +  tx_rx_offset, _commands, _amount_commands);
