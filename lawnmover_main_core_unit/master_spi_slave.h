    ...
    bool interpret_communication(const uint8_t *tx_buffer, const uint8_t *rx_buffer, const long buffer_size) {
        ...
        for (long i = 0; i + COMMAND_FRAME_SIZE <= buffer_size; i += COMMAND_FRAME_SIZE) {
            ...
            if (id1 <= 0 || id2 <= 0) {
                ...
                if (processed) {
                    SerialLogger::trace("Data response from slave %s is %ld for id %d", k_name, rx_data, id1);
                } else {
                    SerialLogger::trace("Data push to slave %s with ack response value %ld for id %d", k_name, rx_data, id1);
        ...
        SerialLogger::trace("Validating master-slave communication for %s done", k_name);
        return true;
    }
