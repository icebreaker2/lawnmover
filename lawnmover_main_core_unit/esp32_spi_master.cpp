...
void Esp32SpiMaster::schedule(const int interval, Timer<> &timer) {
	...
	timer.every(interval,[&](void *) -> bool {
    	if (_registered_slaves < 1) {
    		SerialLogger::error("No slaves registered. No need to start a timer for spi communication for no spi slaves");
    		_stopped = true;
    	} else {
    		...
    		if (tx_rx_buffer_size < 0) {
    			SerialLogger::error("Cannot create spi slave communication. Supplier returned bad buffer_size %d",
    					            tx_rx_buffer_size);
    			_stopped = true;
    		} else if (tx_buffer == nullptr) {
                SerialLogger::error("Cannot create spi slave communication. Tx buffer were supplied as nullptr from slave representation");
                _stopped = true;
            } else if (rx_buffer == nullptr) {
                SerialLogger::error("Cannot create spi slave communication. Rx buffer were supplied as nullptr from slave representation");
                _stopped = true;
            } else {
    			...
    			if (!_stopped) {
    			    if (spi_slave->consume(rx_buffer, tx_rx_buffer_size)) {
                        ...
                        SerialLogger::error("Slave on slave select pin %d (%s) did NOT return correct results!",
                                            spi_slave->get_slave_pin(), spi_slave->get_name());
                        spi_slave->restart();
                    }
    			}
            ...
            if (_stopped) {
                SerialLogger::error("Spi communication appears broken. Stopping scheduler, thus, whole spi communication.");
            }
