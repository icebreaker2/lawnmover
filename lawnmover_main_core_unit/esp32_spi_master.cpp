...
void Esp32SpiMaster::addSlave(const int slave_pin, const int slave_power_pin, const int slave_boot_delay,
                              const int interval, const long clock_divider, Timer<> &timer, uint8_t *(*supplier)(long&),
                              bool(*consumer)(uint8_t *, long)) {
    if (mastersCounter_ < sizeof(masters_) / sizeof(masters_[0])) {
        ESP32DMASPI::Master *master = setup_slave(slave_pin, slave_power_pin, slave_boot_delay, interval);
        ...
        void (*tear_down_slave_pointer)(const int, const int) = &tear_down_slave;
        volatile bool &shutdown = shutdown_;
        timer.every(interval, [&shutdown, slave_pin, slave_power_pin, chunk_size, max_tx_rx_buffer_size, rx_buffer, master, supplier,
        consumer, error_callback, tear_down_slave_pointer](void*) -> bool {
            bool repeat = true;
            if (shutdown) {
                repeat = false;
            } else {
            ...
            if (!repeat) {
                (*tear_down_slave_pointer)(slave_pin, slave_power_pin);
            }
...
ESP32DMASPI::Master *Esp32SpiMaster::setup_slave(const int slave_pin, const int slave_power_pin, const int slave_boot_delay, const int interval) {
    ...
    SerialLogger::info("Adding slave %d/%d", mastersCounter_, sizeof(masters_) / sizeof(masters_[0]));
...
void Esp32SpiMaster::tear_down_slave(const int slave_pin, const int slave_power_pin) {
    SerialLogger::info("Shutting down spi communication to slave connected to slave-select pin %d with power supply on pin %d", slave_pin, slave_power_pin);
